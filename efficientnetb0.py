...
def mbconvblock(inputs, activations='swish', droprate=0., name='', filters_in=32, filters_out=16,
                     kernel_size=3, strides=1, expand_ratio=1, se_ratio=0., id_skip=True, attn=False):
    ...
    if attn :
        ...
        attn_mask = layers.Conv2D(filters=1, kernel_size=1, activation='sigmoid', padding='same', use_bias=False, kernel_initializer=CONV_KERNEL_INITIALIZER)(conc)
    ...
    for (i, args) in enumerate(block_args):
        ...
        for j in range(round_repeats(args.pop('repeats'))):
            ...
            x = mbconvblock(inputs=x, activations=activations,
                droprate=drop_connect_rate * b / blocks, **args)
            b+=1
