...
def block(inputs,activation='swish',drop_rate=0.,name='',filters_in=32,filters_out=16,kernel_size=3,strides=1,expand_ratio=1,se_ratio=0.,id_skip=True,attn=True):
    ...
    #csp split
    x_, x  =  tf.split(x,num_or_size_splits = 2, axis = -1)
    ...
    if 0 < se_ratio <= 1:
        ...
        if bn_axis == 1:
          se_shape = (int(filters/2), 1, 1)
        else:
          se_shape = (1, 1, int(filters/2))
    ...
    if attn :
        ...
        attn_mask = layers.Conv2D(filters=1, kernel_size=1, activation='sigmoid', padding='same', 
                         use_bias=False, kernel_initializer=CONV_KERNEL_INITIALIZER)(conc)
        x = layers.multiply([x, attn_mask])
    
    # Output phase
    if(inputs.shape[1] != x.shape[1]):
        x = layers.Conv2DTranspose(filters=int(filters/2), kernel_size= 1, strides=2, activation=activation, padding='same')(x)
    else:
        x = layers.Conv2DTranspose(filters=int(filters/2), kernel_size= 1, strides=1, activation=activation, padding='same')(x)
    
        #csp concat
    x = layers.Concatenate(axis=3)([x_,x])
